...
> | 阶段 | 状态 |
> |---|---|
> | **S0 Spike** | ✅ 完成 —— 结论推翻了首选方案，见 `NOTES.md` §1 |
> | **M0 骨架 + 只读** | ✅ 完成 |
> | **M1 引用校验** | ✅ 完成 —— 74 个单测，四条判定路径端到端验证 |
> | **M2 度量与打包** | 🟠 **部分完成** —— 打包验证通过；基线 A/B 因供应商配额耗尽只跑了 A 2/12、B 7/12 |
> | **交互式验收** | ✅ 2026-09-11 在真实 TUI 中完成，见 `NOTES.md` §8 |
>
> **本文档是设计意图的记录，不是实际结果的记录。**
> 实际做成什么样、还有哪些局限 → 看 [`README.md`](README.md) 的「现状与局限」；实测数据 → 看 [`NOTES.md`](NOTES.md)；自己验收 → 看 [`VERIFY.md`](VERIFY.md)。
...
- [ ] 通过用户 `settings.json` 的 `"extensions": ["/绝对路径/pi-grounded/src/index.ts"]` 加载后 `/reload` 正常，无残留状态
