...
## 8. 交互式验收（2026-09-11，Kayce001 在真实 TUI 中执行）

交付时 `HANDOFF.md` §3 列了 8 条未验证假设，全部因为无头模式无法派发斜杠命令、UI 方法是 no-op。本节是补上的结果。
...
题目：让模型重排一段含三处引用的用户笔记（工作目录 `<pi-src>`）。
...
**经过**：在 `<repo>` 目录**裸跑** pi（忘了 `-e`），敲 `/grounded`。pi 不认识这个命令，当作普通消息发给模型。模型先 `read` 了 `src/index.ts` 和 README，然后输出：
